...
import type { Module, Definition, BinaryExpression, FunctionCall } from '../src/language-server/generated/ast.js';
...
describe('Arithmetics validation (Lezer backend)', () => {
    ...
    test('Duplicate parameter names should be detected', async () => {
        ...
        // Verify both duplicate parameters are flagged.
        // Note: Lezer's SyntaxNode doesn't support CstNode-based findNodeForProperty,
        // so we check diagnostics by count and message rather than exact node matching.
        expect(validationResult.diagnostics).toHaveLength(2);
        expect(validationResult.diagnostics.every(d => d.message === 'Duplicate definition name: x')).toBe(true);
    });
