...
import type { Grammar, Stream } from 'langium-core';
import { GrammarAST, AstUtils, GrammarUtils, RegExpUtils, stream } from 'langium-core';
import { Lexer } from 'chevrotain';
import { partialMatches } from '../utils/chevrotain-regexp-utils.js';

// GrammarAST type aliases
type AbstractRule = GrammarAST.AbstractRule;
type Keyword = GrammarAST.Keyword;
type TerminalRule = GrammarAST.TerminalRule;

// Runtime values
const { isAbstractParserRule, isKeyword, isTerminalRule } = GrammarAST;
const { streamAllContents } = AstUtils;
const { getAllReachableRules, terminalRegex } = GrammarUtils;
const { escapeRegExp, isWhitespace } = RegExpUtils;
