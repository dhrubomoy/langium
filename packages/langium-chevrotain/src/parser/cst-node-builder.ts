...
import type { AstNode, CompositeCstNode, CstNode, CstTokenType, LeafCstNode, RootCstNode, GrammarAST } from 'langium-core';
import { wrapCstNode, wrapRootCstNode, CstUtils } from 'langium-core';
type AbstractElement = GrammarAST.AbstractElement;
...
export class CstNodeBuilder {
    ...
    buildLeafNode(token: IToken, feature?: AbstractElement): LeafCstNode {
        const leafNode = new LeafCstNodeImpl(token.startOffset, token.image.length, CstUtils.tokenToRange(token), token.tokenType, !feature);
    ...
    addHiddenNodes(tokens: IToken[]): void {
        ...
        for (const token of tokens) {
            const leafNode = new LeafCstNodeImpl(token.startOffset, token.image.length, CstUtils.tokenToRange(token), token.tokenType, true);
            leafNode.root = this.rootNode;
            nodes.push(leafNode);
        }
    ...
    construct(item: { $type: string | symbol | undefined, $cstNode: CstNode, $infixName?: string }): void {
        ...
        // Define a lazy $syntaxNode getter that wraps the CstNode on first access.
        // Uses wrapCstNode/wrapRootCstNode with WeakMap cache to avoid duplicate allocations.
        // Root CstNodes (which have fullText) are wrapped as RootSyntaxNode.
        Object.defineProperty(item, '$syntaxNode', {
            get() {
                if (!item.$cstNode) return undefined;
                if ('fullText' in item.$cstNode) {
                    return wrapRootCstNode(item.$cstNode as RootCstNode);
                }
                return wrapCstNode(item.$cstNode);
            },
            configurable: true,
            enumerable: true
        });
...
export class LeafCstNodeImpl extends AbstractCstNode implements LeafCstNode {
    ...
    private _tokenType: CstTokenType;

    constructor(offset: number, length: number, range: Range, tokenType: CstTokenType, hidden = false) {
        super();
        this._hidden = hidden;
        this._offset = offset;
