...
import type { AstNode, AstReflection, CompositeCstNode, CstNode, ParseResult, ParserOptions, ProfilingTask, Linker, ValueConverter } from 'langium-core';
import { GrammarAST, AstUtils, GrammarUtils } from 'langium-core';
import type { DSLMethodOpts, IOrAlt, IParserErrorMessageProvider, IToken, ParserMethod, SubruleMethodOpts, TokenType, TokenVocabulary, IRuleConfig } from 'chevrotain';
...
// Re-export the backend-agnostic ParseResult from the core file
export type { ParseResult, ParseError, LexError, LexReport, LexDiagnostic, LexDiagnosticSeverity, ErrorToken, ParserOptions } from 'langium-core';

// GrammarAST type aliases for readability
type AbstractElement = GrammarAST.AbstractElement;
type Action = GrammarAST.Action;
type Assignment = GrammarAST.Assignment;
type InfixRule = GrammarAST.InfixRule;
type ParserRule = GrammarAST.ParserRule;

// Runtime values from GrammarAST
const { isInfixRule, isAssignment, isCrossReference, isKeyword, isParserRule } = GrammarAST;
// Utility functions
const { getTypeName, isDataTypeRule } = GrammarUtils;
const { assignMandatoryProperties, getContainerOfType, linkContentToContainer } = AstUtils;
