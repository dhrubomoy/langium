...
import type { AstNode, Grammar } from 'langium-core';
import { GrammarAST, GrammarUtils, assertUnreachable, ErrorWithLocation, stream } from 'langium-core';
import type { BaseParser } from './langium-parser.js';
import { EMPTY_ALT, EOF } from 'chevrotain';

// GrammarAST type aliases
type AbstractElement = GrammarAST.AbstractElement;
type Action = GrammarAST.Action;
type Alternatives = GrammarAST.Alternatives;
type Assignment = GrammarAST.Assignment;
type Condition = GrammarAST.Condition;
...
type NamedArgument = GrammarAST.NamedArgument;
type ParserRule = GrammarAST.ParserRule;
type RuleCall = GrammarAST.RuleCall;
type UnorderedGroup = GrammarAST.UnorderedGroup;
type Cardinality = GrammarUtils.Cardinality;

// Runtime values from GrammarAST
const { isAction, isAlternatives, isEndOfFile, isAssignment, isConjunction, isCrossReference, isDisjunction, isGroup, isKeyword, isNegation, isParameterReference, isParserRule, isRuleCall, isTerminalRule, isUnorderedGroup, isBooleanLiteral, isInfixRule, isAbstractParserRule } = GrammarAST;
const { findNameAssignment, getAllReachableRules, getTypeName } = GrammarUtils;
