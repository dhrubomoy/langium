...
import type { ErrorToken, LexDiagnostic, LexDiagnosticSeverity, ParseResult } from '../parser/parse-result.js';
...
export class DefaultDocumentValidator implements DocumentValidator {
    ...
    protected processLexingErrors(parseResult: ParseResult, diagnostics: Diagnostic[], _options: ValidationOptions): void {
        const lexerDiagnostics: LexDiagnostic[] = [...parseResult.lexerErrors, ...parseResult.lexerReport?.diagnostics ?? []];
        for (const lexerDiagnostic of lexerDiagnostics) {
            const severity: LexDiagnosticSeverity = lexerDiagnostic.severity ?? 'error';
    ...
    protected processParsingErrors(parseResult: ParseResult, diagnostics: Diagnostic[], _options: ValidationOptions): void {
        for (const parserError of parseResult.parserErrors) {
            ...
            // We can run into the parser error recovery here
            // The token contained in the parser error might be automatically inserted
            // In this case every position value will be `NaN`
            if (isNaN(parserError.token.startOffset)) {
                // Some special parser error types contain a `previousToken`
                // We can simply append our diagnostic to that token
                if (parserError.previousToken) {
                ...
                range = errorTokenToRange(parserError.token);
            }
            if (range) {
                const diagnostic: Diagnostic = {
...
/**
 * Converts an ErrorToken to an LSP Range.
 * Chevrotain uses 1-based line/column values, so we subtract 1 to align with the LSP (0-based).
 */
function errorTokenToRange(token: ErrorToken): Range {
    return {
        start: {
...
/**
 * Transforms the diagnostic severity from the {@link LexDiagnosticSeverity} format to LSP's `DiagnosticSeverity` format.
 *
 * @param severity The lexing diagnostic severity
 * @returns Diagnostic severity according to `vscode-languageserver-types/lib/esm/main.js#DiagnosticSeverity`
 */
export function toDiagnosticSeverity(severity: LexDiagnosticSeverity): DiagnosticSeverity {
    switch (severity) {
        case 'error':
...
export function toDiagnosticData(severity: LexDiagnosticSeverity): DiagnosticData {
    switch (severity) {
        case 'error':
