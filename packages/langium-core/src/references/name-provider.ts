...
export interface NameProvider {
     ...
     * @param node Specified `AstNode` whose name node shall be retrieved.
     * @deprecated Use {@link getNameSyntaxNode} instead.
     */
    getNameNode(node: AstNode): CstNode | undefined;
    /**
     * Returns the `SyntaxNode` which contains the parsed value of the `name` assignment.
     * @param node Specified `AstNode` whose name syntax node shall be retrieved.
     */
    getNameSyntaxNode(node: AstNode): SyntaxNode | undefined;
}
...
export class DefaultNameProvider implements NameProvider {
    ...
    getNameSyntaxNode(node: AstNode): SyntaxNode | undefined {
        return findNodeForPropertySN(node.$syntaxNode, 'name');
    }
}
