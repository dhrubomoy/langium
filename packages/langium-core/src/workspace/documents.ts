    ...
    protected createTextDocumentGetter(uri: URI, text?: string): () => TextDocument {
        ...
        return () => {
            if (!textDoc) {
                ...
                try {
                    languageId = serviceRegistry.getServices(uri).LanguageMetaData.languageId;
                } catch {
                    // Service registry may be empty for programmatically created documents
                    // (e.g. grammars loaded from JSON via loadGrammarFromJson).
                }
                textDoc = TextDocument.create(uri.toString(), languageId, 0, text ?? '');
            }
            return textDoc;
        };
    }
