...
export interface Formatter {
     ...
     */
    formatDocument(document: LangiumDocument, params: DocumentFormattingParams, cancelToken?: Cancellation.CancellationToken): MaybePromise<TextEdit[]>
    /**
     * Handles partial document formatting. Only parts of the document within the `params.range` property are formatted.
     */
    formatDocumentRange(document: LangiumDocument, params: DocumentRangeFormattingParams, cancelToken?: Cancellation.CancellationToken): MaybePromise<TextEdit[]>
    /**
     * Handles document formatting while typing. Only formats the current line.
     */
    formatDocumentOnType(document: LangiumDocument, params: DocumentOnTypeFormattingParams, cancelToken?: Cancellation.CancellationToken): MaybePromise<TextEdit[]>
    /**
    ...
    protected iterateAstFormatting(document: LangiumDocument, range?: Range): void {
        ...
        do {
            ...
            if (!result.done) {
                ...
                const inside = this.insideRange((node.$syntaxNode ?? node.$cstNode)!.range, range);
                if (inside) {
                    this.format(node);
                } else {
                    treeIterator.prune();
