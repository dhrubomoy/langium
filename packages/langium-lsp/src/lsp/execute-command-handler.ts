...
import type { MaybePromise } from 'langium-core';
import { Cancellation } from 'langium-core';

export interface ExecuteCommandHandler {
    get commands(): string[]
    executeCommand(name: string, args: any[], cancelToken?: Cancellation.CancellationToken): Promise<unknown>
}

export type ExecuteCommandFunction = (args: any[], cancelToken: Cancellation.CancellationToken) => MaybePromise<unknown>
...
export abstract class AbstractExecuteCommandHandler implements ExecuteCommandHandler {
    ...
    async executeCommand(name: string, args: any[], cancelToken = Cancellation.CancellationToken.None): Promise<unknown> {
