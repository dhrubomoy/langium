...
import type { AstNode, Properties, SyntaxNode, MaybePromise, LangiumDocument } from 'langium-core';
import { AstUtils, Cancellation, CstUtils, SyntaxNodeUtils, interruptAndCheck } from 'langium-core';
...
export interface SemanticTokenProvider {
    semanticHighlight(document: LangiumDocument, params: SemanticTokensParams, cancelToken?: Cancellation.CancellationToken): MaybePromise<SemanticTokens>
    semanticHighlightRange(document: LangiumDocument, params: SemanticTokensRangeParams, cancelToken?: Cancellation.CancellationToken): MaybePromise<SemanticTokens>
    semanticHighlightDelta(document: LangiumDocument, params: SemanticTokensDeltaParams, cancelToken?: Cancellation.CancellationToken): MaybePromise<SemanticTokens | SemanticTokensDelta>
...
export abstract class AbstractSemanticTokenProvider implements SemanticTokenProvider {
    ...
    async semanticHighlight(document: LangiumDocument, _params: SemanticTokensParams, cancelToken = Cancellation.CancellationToken.None): Promise<SemanticTokens> {
    ...
    async semanticHighlightRange(document: LangiumDocument, params: SemanticTokensRangeParams, cancelToken = Cancellation.CancellationToken.None): Promise<SemanticTokens> {
    ...
    async semanticHighlightDelta(document: LangiumDocument, params: SemanticTokensDeltaParams, cancelToken = Cancellation.CancellationToken.None): Promise<SemanticTokens | SemanticTokensDelta> {
        this.currentRange = undefined;
        this.currentDocument = document;
        this.currentTokensBuilder = this.getDocumentTokensBuilder(document);
    ...
    protected async computeHighlighting(document: LangiumDocument, acceptor: SemanticTokenAcceptor, cancelToken: Cancellation.CancellationToken): Promise<void> {
    ...
    protected highlightProperty<N extends AstNode>(options: SemanticTokenPropertyOptions<N>): void {
        ...
        if (typeof options.index === 'number') {
            const node = SyntaxNodeUtils.findNodeForPropertySN(options.node.$syntaxNode, options.property, options.index);
            if (node) {
                nodes.push(node);
            }
        } else {
            nodes.push(...SyntaxNodeUtils.findNodesForPropertySN(options.node.$syntaxNode, options.property));
        }
    ...
    protected highlightKeyword(options: SemanticTokenKeywordOptions): void {
        ...
        if (typeof index === 'number') {
            const keywordNode = SyntaxNodeUtils.findNodeForKeywordSN(node.$syntaxNode, keyword, index);
            if (keywordNode) {
                nodes.push(keywordNode);
            }
        } else {
            nodes.push(...SyntaxNodeUtils.findNodesForKeywordSN(node.$syntaxNode, keyword));
        }
