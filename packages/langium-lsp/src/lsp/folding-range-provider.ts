...
export interface FoldingRangeProvider {
     ...
     */
    getFoldingRanges(document: LangiumDocument, params: FoldingRangeParams, cancelToken?: Cancellation.CancellationToken): MaybePromise<FoldingRange[]>;
}
...
export class DefaultFoldingRangeProvider implements FoldingRangeProvider {
    ...
    getFoldingRanges(document: LangiumDocument, _params: FoldingRangeParams, _cancelToken?: Cancellation.CancellationToken): MaybePromise<FoldingRange[]> {
    ...
    protected collectObjectFolding(document: LangiumDocument, node: AstNode, acceptor: FoldingRangeAcceptor): void {
        ...
        if (syntaxNode) {
            const foldingRange = this.toFoldingRange(document, syntaxNode);
            if (foldingRange) {
    ...
    protected collectCommentFolding(document: LangiumDocument, node: AstNode, acceptor: FoldingRangeAcceptor): void {
        const syntaxNode = node.$syntaxNode;
        if (syntaxNode) {
            for (const node of SyntaxNodeUtils.flattenSyntaxTree(syntaxNode)) {
                if (node.tokenType !== undefined && this.commentNames.includes(node.tokenType)) {
    ...
    protected toFoldingRange(document: LangiumDocument, node: SyntaxNode, kind?: string): FoldingRange | undefined {
     ...
     */
    protected includeLastFoldingLine(node: SyntaxNode, kind?: string): boolean {
