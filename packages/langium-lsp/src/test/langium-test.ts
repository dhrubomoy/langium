...
import { Disposable, GrammarUtils, RegExpUtils, stream, URI, DocumentValidator, TextDocument } from 'langium-core';
import { normalizeEOL } from 'langium-core/generate';
import type { ParserOptions, LangiumCoreServices, LangiumSharedCoreServices, AstNode, CstNode, Properties, AsyncDisposable, BuildOptions, LangiumDocument } from 'langium-core';
...
export function replaceIndices(base: ExpectedBase): { output: string, indices: number[], ranges: Array<[number, number]> } {
    ...
    const regex = new RegExp(`${RegExpUtils.escapeRegExp(indexMarker)}|${RegExpUtils.escapeRegExp(rangeStartMarker)}|${RegExpUtils.escapeRegExp(rangeEndMarker)}`);
...
export function filterByOptions<T extends AstNode = AstNode, N extends AstNode = AstNode>(validationResult: ValidationResult<T>, options: ExpectDiagnosticOptions<N>) {
    ...
    if ('node' in options && options.node) {
        ...
        if (options.property) {
            ...
            cstNode = GrammarUtils.findNodeForProperty(cstNode, name, index);
        }
        if (!cstNode) {
