...
export class LangiumGrammarFormatter extends AbstractFormatter {
    ...
    protected format(node: AstNode): void {
        if (GrammarAST.isCrossReference(node)) {
            const formatter = this.getNodeFormatter(node);
            formatter.properties('type', 'terminal').surround(Formatting.noSpace());
        } else if (GrammarAST.isParserRule(node)) {
            const formatter = this.getNodeFormatter(node);
        ...
        } else if (GrammarAST.isTerminalRule(node)) {
            const formatter = this.getNodeFormatter(node);
            if (node.type) {
                formatter.property('name').append(Formatting.oneSpace());
            ...
            formatter.keyword('}').prepend(Formatting.noSpace());
        } else if (GrammarAST.isInferredType(node)) {
            const formatter = this.getNodeFormatter(node);
            formatter.keywords('infer', 'infers').append(Formatting.oneSpace());
        } else if (GrammarAST.isAssignment(node)) {
            const formatter = this.getNodeFormatter(node);
            formatter.keywords('=', '+=', '?=').surround(Formatting.noSpace());
        ...
        } else if (GrammarAST.isInterface(node)) {
            const formatter = this.getNodeFormatter(node);
        ...
        } else if (GrammarAST.isType(node)) {
            const formatter = this.getNodeFormatter(node);
        ...
        } else if (GrammarAST.isGrammar(node)) {
            const formatter = this.getNodeFormatter(node);
        ...
        if (GrammarAST.isAbstractElement(node)) {
            const formatter = this.getNodeFormatter(node);
