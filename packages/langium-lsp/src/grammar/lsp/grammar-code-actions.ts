...
import { GrammarAST, AstUtils, CstUtils, RegExpUtils, UriUtils, DocumentValidator } from 'langium-core';
import type { AstReflection, Reference, ReferenceInfo, MaybePromise, URI, LinkingErrorData, DiagnosticData, LangiumDocument, IndexManager } from 'langium-core';
import { IssueCodes } from 'langium-core/grammar';
...
export class LangiumGrammarCodeActionProvider implements CodeActionProvider {
    ...
    getCodeActions(document: LangiumDocument<GrammarAST.Grammar>, params: CodeActionParams): MaybePromise<Array<Command | CodeAction>> {
    ...
    private createCodeActions(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>, accept: (ca: CodeAction | undefined) => void): void {
     ...
     */
    private fixMissingReturns(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
    ...
    private fixInvalidReturnsInfers(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
    ...
    private fixMissingInfer(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
        const data = diagnostic.data as DiagnosticData;
        if (data && data.actionSegment) {
            return {
    ...
    private fixMissingCrossRefTerminal(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
        const grammar = document.parseResult.value;
        const idTerminal = grammar.rules.find(rule => GrammarAST.isTerminalRule(rule) && rule.name === 'ID');
        if (idTerminal) {
            return {
    ...
    private fixSuperfluousInfer(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
        const data = diagnostic.data as DiagnosticData;
    ...
    private isRuleReplaceable(rule: GrammarAST.ParserRule): boolean {
    ...
    private replaceRule(rule: GrammarAST.ParserRule): string {
        const type = rule.inferredType ?? rule;
        return type.name;
    }
    private isDefinitionReplaceable(node: GrammarAST.AbstractElement): boolean {
        if (GrammarAST.isRuleCall(node)) {
            return node.arguments.length === 0 && GrammarAST.isParserRule(node.rule.ref) && this.isRuleReplaceable(node.rule.ref);
        }
        if (GrammarAST.isAlternatives(node)) {
            return node.elements.every(child => this.isDefinitionReplaceable(child));
        }
        return false;
    }
    private replaceDefinition(node: GrammarAST.AbstractElement): string {
        if (GrammarAST.isRuleCall(node) && node.rule.ref) {
    ...
    private replaceParserRuleByTypeDeclaration(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
        const rootCst = document.parseResult.value.$cstNode;
        if (rootCst) {
            const offset = document.textDocument.offsetAt(diagnostic.range.start);
            const cstNode = CstUtils.findLeafNodeAtOffset(rootCst, offset);
            const rule = AstUtils.getContainerOfType(cstNode?.astNode, GrammarAST.isParserRule);
    ...
    private fixUnnecessaryFileExtension(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction {
    ...
    private makeUpperCase(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction {
        const range = {
    ...
    private addEntryKeyword(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
        return {
    ...
    private fixRegexTokens(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
        const offset = document.textDocument.offsetAt(diagnostic.range.start);
        const rootCst = document.parseResult.value.$cstNode;
        if (rootCst) {
            const cstNode = CstUtils.findLeafNodeAtOffset(rootCst, offset);
            const container = AstUtils.getContainerOfType(cstNode?.astNode, GrammarAST.isCharacterRange);
                    ...
                    edit: {
                        changes: {
                            [document.textDocument.uri]: [{
                                ...
                                newText: `/[${RegExpUtils.escapeRegExp(left)}-${RegExpUtils.escapeRegExp(right)}]/`
                            }]
                        }
                    }
                };
    ...
    private fixCrossRefSyntax(diagnostic: Diagnostic, document: LangiumDocument<GrammarAST.Grammar>): CodeAction {
        return {
            title: "Replace '|' with ':'",
            kind: CodeActionKind.QuickFix,
            diagnostics: [diagnostic],
    ...
    private addNewRule(diagnostic: Diagnostic, data: LinkingErrorData, document: LangiumDocument<GrammarAST.Grammar>): CodeAction | undefined {
        const offset = document.textDocument.offsetAt(diagnostic.range.start);
        const rootCst = document.parseResult.value.$cstNode;
        if (rootCst) {
            const cstNode = CstUtils.findLeafNodeAtOffset(rootCst, offset);
            const container = AstUtils.getContainerOfType(cstNode?.astNode, GrammarAST.isParserRule);
    ...
    private lookInGlobalScope(diagnostic: Diagnostic, data: LinkingErrorData, document: LangiumDocument<GrammarAST.Grammar>): CodeAction[] {
        ...
        for (const candidate of candidates) {
            ...
            let suffix = '';
            const grammar = document.parseResult.value as GrammarAST.Grammar;
