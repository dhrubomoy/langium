...
export class LezerGrammarTranslator implements GrammarTranslator {
    ...
    async translate(grammar: Grammar, outputDir: string): Promise<TranslationResult> {
        ...
        try {
            ...
            // Write case-insensitive keywords module
            if (this.caseInsensitive) {
                const identKeywords = [...keywords].filter(kw => /^[_a-zA-Z]\w*$/.test(kw));
                if (identKeywords.length > 0) {
                    const kwModulePath = path.join(outputDir, `${languageId}.keywords.ts`);
                    const kwModuleCode = this.generateKeywordsModule(languageId, identKeywords);
                    await fs.promises.writeFile(kwModulePath, kwModuleCode, 'utf-8');
                    outputFiles.push(kwModulePath);
                }
            }
        } catch (e) {
            diagnostics.push({
    ...
    private generateKeywordsModule(languageId: string, identKeywords: string[]): string {
        const kwListStr = identKeywords.map(kw => `'${kw}'`).join(', ');
        return [
            '/******************************************************************************',
            ' * DO NOT EDIT MANUALLY!',
            ' * This file was generated by langium-lezer for case-insensitive keyword matching.',
            ' ******************************************************************************/',
            `import * as terms from './${languageId}.terms.js';`,
            '',
            `const kwList = [${kwListStr}];`,
    ...
    private translateKeyword(kw: GrammarAST.Keyword, keywords: Set<string>): string {
        ...
        if (/^[_a-zA-Z]\w*$/.test(kw.value)) {
            // In case-insensitive mode, reference the externally-specialized token directly
            if (this.caseInsensitive) {
                return kw.value;
            }
